...
my $action_by_type_and_name = sub {
    ...
    return $controller->resource_not_found unless $controller->daemon->{core}->job_type_exists($type);

    my $job = $controller->daemon->{core}->job_by_type_and_name($type, $name);

    return $controller->resource_not_found($name) unless defined $job;
...
sub show_by_type_and_name {
    ...
    return $controller->resource_not_found unless $controller->daemon->{core}->job_type_exists($job_properties{type});

    my $job = $controller->daemon->{core}->job_by_type_and_name($job_properties{type}, $job_properties{name});

    return $controller->resource_not_found($job_properties{name}) unless defined $job;
